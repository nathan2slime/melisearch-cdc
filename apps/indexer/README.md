...
For Docker Compose, copy the root `.env.example` to `.env.production`. The template uses Docker-network values, like `KAFKA_BOOTSTRAP_SERVERS=kafka:19092` and `MEILISEARCH_URL=http://meilisearch:7700`.
